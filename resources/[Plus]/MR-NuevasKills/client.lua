...
local pane = guiCreateWindow(panelX, panelY, panelW, panelH, 'Kills Recientes - Administracion Majestic RP', false)
if not pane then
  return false
end
...
addEventHandler('onClientPreRender', root, function()
    if getTickCount() - pointerTick > 1000 then
      ...
      if pointerCount == 3 then
        guiSetText(loadingText, 'Cargando datos increíbles...')
        pointerCount = 0
      end
...
guiGridListAddColumn(killerList, 'ID', 0.2)
guiGridListAddColumn(killerList, 'Jugador', 0.6)
guiGridListAddColumn(killerList, 'Asesino', 0.15)
