...
function resetTimeAFK()
	infos = 0
end

for k, v in ipairs(afkKeys) do
	bindKey(v, "both", resetTimeAFK)
end

function noAFK()
	if getTickCount() - getTickCount() > 60000 then
		if infos > 3 then
			localPlayer:triggerEvent("kickAFK", localPlayer)
		else
			if infos >= 1 then
				mensaje = "[Servidor] Estas AFK"
			else
				mensaje = "[Servidor] Sigues estando AFK"
			end
			infos = infos + 1
...
local refreshTimer = setTimer(noAFK, 60000, 0)