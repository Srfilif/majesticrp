...
addEventHandler("onClientGUIDoubleClick", resourceRoot, function()
    ...
    if source == ListaModificaciones then
        ...
        if mod ~="" then
            ...
            elseif mod == "Nitro" then
-- Obtener el vehículo ocupado por el jugador local
local player = getLocalPlayer()
buyNitroCommand(player)
-------------------------------
            end
...
-- Archivo: buynitro.lua


function buyNitroCommand(player)
    -- Verifica si el jugador tiene suficiente dinero para comprar nitro (1000 unidades)
player = getLocalPlayer()
    local money = getPlayerMoney(player)
    if money >= 1000 then
        -- Obtiene el vehículo del jugador
        local vehicle = getPedOccupiedVehicle(player)

        -- Verifica si el jugador está dentro de un vehículo
        if vehicle then
            -- Aumenta el nivel de nitro en el vehículo (o instala nitro si no está presente)
            local currentNitroLevel = getVehicleNitroLevel(vehicle) or 0.1
            local newNitroLevel = currentNitroLevel + 0.1

            -- Calcula el precio basado en el nivel actual de nitro
            local price = 10000

            -- Resta el precio del dinero del jugador
            takePlayerMoney(price)
             local vehicleName = getVehicleName ( vehicle )
            -- Establece el nuevo nivel de nitro en el vehículo
            setVehicleNitroLevel(vehicle, newNitroLevel)

            -- Envía un mensaje al jugador indicando que ha comprado nitro
            outputChatBox("#ebcc34[MECANICO] #ffFFffHas comprado nitro para tu "..vehicleName.." por #00ff00" .. price .. "$#ffFFff dolares.",255,255,255,true)
        else
            -- Envía un mensaje al jugador si no está en un vehículo
            outputChatBox("Debes estar dentro de un vehículo para comprar nitro.",255,255,255,true)
        end
    else
        -- Envía un mensaje al jugador si no tiene suficiente dinero
        outputChatBox("No tienes suficiente dinero para comprar nitro.",255,255,255,true)
    end
end

-- Asocia la función al comando /buynitro
addCommandHandler("buynitro", buyNitroCommand)
