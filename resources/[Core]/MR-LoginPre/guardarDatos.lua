...
function LoginSaveP( p, c, a )
	if not(mysql:notIsGuest(source)) then 
		...
		--
		local x, y, z = 1743.3916015625, -1860.642578125, 13.57848739624
		...
		local skin = 1
	    local Creacion = source:getData("EnCreacionPJ")
		...
		if ( type ( save ) == "table" and #save == 0 ) or not save then
			...
			source:spawn( 223.38671875, 1292.9267578125, 1082.140625, 180, 1, 1, 0)
			source:setHealth(vida)
			source:setArmor(chaleco)
			source:giveMoney(1000000)
			--
			source:fadeCamera(true, 0.5)
			source:setCameraTarget(source)
			source:setTeam(nil)
			source:setData("EnEdicion", true)
			source:setData("EnCreacionPJ", true)
			showChat(source, false)
			setTimer(triggerClientEvent, 800, 1, source, "setWindowSkin", source, true)
			setTimer(setCameraMatrix, 500, 1, source, 223.3955078125, 1290.396484375, 1082.1328125, 223.3955078125, 1290.396484375, 1082.1328125)
		else
			...
			elseif exports["MR-LFacciones"]:isPlayerExistsArresto(source) then
				exports["MR-LFacciones"]:setPlayerJailPolice(source)
