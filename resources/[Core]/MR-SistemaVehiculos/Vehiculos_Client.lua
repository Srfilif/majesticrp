...
--	outputChatBox ( ""..atList.."" )
	...
	priceList = guiGridListAddColumn( vehicleList, "Precios", 0.4 )
	Slots = guiGridListAddColumn( vehicleList,"Slots" ,0.5)
...
function aClickGuiCar (button)
	if (button == "left" ) then
		if (source == vehicleList ) then
			...
			if ( carPic ) then
				if guiStaticImageLoadImage ( carPic, "Png\\"..aCurrentVehicle..".png" ) == false then
					guiStaticImageLoadImage ( carPic, "Png\\rien.png" )
				end
			else
	...
	for theKey,colshapeComp in ipairs(colshapeComps) do
		if getElementData ( colshapeComp, "SpawnPointName" ) == shopName then
			...
			if spawnState == "libre" or spawnState == false then
				aCurrentVehicle = guiGridListGetItemText ( vehicleList, guiGridListGetSelectedItem ( vehicleList ), 1 )
				local cost = guiGridListGetItemText ( vehicleList, guiGridListGetSelectedItem ( vehicleList ), 2 )
				local male = guiGridListGetItemText ( vehicleList, guiGridListGetSelectedItem ( vehicleList ), 3 )
				local vId = getVehicleModelFromName ( aCurrentVehicle )
...
--					end
					triggerServerEvent ( "but_applyVehicle", getRootElement(), player, cost, vId, x, y, z, nPlayer,tonumber(male), sRz, cColor1, cColor2, cColor3, cColor4)
					aCurrentVehicle = nil
				end
			else
...
function ()
  dxDrawTextOnElement(conceJeff,"",0,20,255,255,255,255,2,"default")
end)
...
addEventHandler("MaleteroAbrir", root, function(cx, cy, cz)
    playSound3D("sonidos/maletero.mp3", cx, cy, cz)
end)

addEvent("MaleteroCerrar", true)
addEventHandler("MaleteroCerrar", root, function(cx, cy, cz)
    playSound3D("sonidos/maletero2.mp3", cx, cy, cz)
end)



addEvent("BloquearVeh", true)
addEventHandler("BloquearVeh", root, function(cx, cy, cz)
    playSound3D("sonidos/bloqueo.mp3", cx, cy, cz)
end)


addEvent("Vehiculos:Reparar",true) 
addEventHandler("Vehiculos:Reparar",localPlayer, 
function () 
local sound = playSound("sonidos/reparar.ogg") 
setSoundVolume(sound, 1) 
end)



addEvent("Vehiclos:Bloqueo",true) 
addEventHandler("Vehiclos:Bloqueo",localPlayer, 
function () 
local sound = playSound("sonidos/bloqueo.mp3") 
setSoundVolume(sound, 1) 
end)

addEvent("Vehiculos:CerrarMaletero",true) 
addEventHandler("Vehiculos:CerrarMaletero",localPlayer, 
function () 
local sound = playSound("sonidos/maletero2.mp3") 
setSoundVolume(sound, 1) 
end)

addEvent("Vehiculos:AbrirMaletero",true) 
addEventHandler("Vehiculos:AbrirMaletero",localPlayer, 
function () 
local sound = playSound("sonidos/maletero.mp3") 
setSoundVolume(sound, 1) 
end)

addEvent("Vehiculos:EncederVeh",true) 
addEventHandler("Vehiculos:EncederVeh",localPlayer, 
function () 
local sound = playSound("sonidos/motor.mp3") 
setSoundVolume(sound, 1) 
end)
addEvent("Vehiculos:NoArranca",true) 
addEventHandler("Vehiculos:NoArranca",localPlayer, 
function () 
local sound = playSound("sonidos/noarranca.ogg") 
setSoundVolume(sound, 1) 
end)
