...
local Z_OFFSET = -20	-- Distance between Notifications
local VEH_OFFSET = 170	-- Offset of Vehicle HUD
...
function renderDXNotification()
	...
	for i,id in ipairs(DXNoteSorted) do
		...
		if (not v[5] or v[5] > getTickCount()) then
			...
			dxDrawText(shadow, x+1, y+1, x+1, y+1, tocolor(0, 0, 0, ALPHA), 1, "default-bold", "right", "bottom", false, false, false, false, true)
			dxDrawText(v[1], x, y, x, y, tocolor(v[2], v[3], v[4], 255), 1, "default-bold", "right", "bottom", false, false, false, true, true)
		else
			DXNotifications[id] = nil
			table.remove(DXNoteSorted, i)
		end
