...
function showPanel(ranks, color, divisions)
    ...
    if (not ranks["duty"] or ranks["duty"] == 0) then
        guiSetText(progressGUI.button[1], "Comenzar Trabajo")
    else
        guiSetText(progressGUI.button[1], "Terminar Trabajo")
    end
...
function renderDisplays()
    ...
    elseif (guiGetSelectedTab(progressGUI.tabpanel[1]) == progressGUI.tab[2]) then
        ...
        dxDrawRectangle(dX, dY, dW, dH, tocolor(255*(1/3),200*(1/3),0*(1/3),200), true)
