...
function sendGUIJobData(job, rank, job_data, rank_data)
		-- Job Name
	jobName = job
	local employer = job_data.employer or "Alcaldia Lakeside Valley"
	guiSetText(employment.window[1], ". "..job.." ~ Contrato de Trabajo — "..employer)
	...
	guiSetText(employment.label[17], desc)
		...
		-- Set App Color
	--[[local team = getTeamFromName(job_data.team) or "Civiles"
...
function setPlayerJob(button, state)
	...
	if (not selSkin) then
		exports.GTIhud:dm("Selecciona una skin de la lista", 255, 125, 0)
		return 
	end
	
	local item = guiComboBoxGetSelected(employment.combobox[1])
	if (not item or item == -1) then 
		exports.GTIhud:dm("Selecciona una division de la lista", 255, 125, 0)
		return
	end
