...
function resourceStarted( )
	...
	for k,v in ipairs( getElementsByType( "player" ) ) do
		...
		if mta_account and isGuestAccount( mta_account ) ~= true then
			local player_balance = getAccountData( mta_account, "Roleplay:bank_balance" );
...
function bank_savePlayerMoney( player, playerAccount )
	if getElementType( player ) == "player" and isGuestAccount( playerAccount ) ~= true then
		setAccountData( playerAccount, "Roleplay:bank_balance", tostring( playersAccount[ player ].balance ) );
...
function bank_playerLogin( oldAccount, currentAccount )
	...
	if isGuestAccount( oldAccount ) == true and getElementType( source ) == "player" then
		local player_balance = getAccountData( currentAccount, "Roleplay:bank_balance" );
		if player_balance then
...
function playerEnterMarker( marker )
	if ( not isPedOnGround ( source ) ) or ( isPedWearingJetpack ( source ) ) or 
		...
		if ( ( not isGuestAccount( account ) ) and ( isObjectInACLGroup( "user." .. getAccountName( account ), aclGetGroup( "Admin" ) ) ) ) then
			...
			if bank_accounts then
				for row in pairs( bank_accounts ) do
					...
					if setAccountData( mta_account, "Roleplay:bank_balance", accountBalance ) then
...
function depositMoney(player, amount)
    ...
    if antiSpamComando[player] and antiSpamComando[player][1] and tick - antiSpamComando[player][1] < 1000 then
        player:outputChat("Debes esperar 1 segundo después de ser utilizado.", 150, 0, 0)
        return
    end

  --  local hasCreditCard = player:getData("Roleplay:tarjeta_credito")

 --   if hasCreditCard == 1 then
        if amount and tonumber(amount) and not string.find(amount, "-") then
            if player:getMoney() >= tonumber(amount) then
                if player:getMoney() >= 1 then
                    local account = player:getAccount()
                    local currentBalance = player:getData("Roleplay:bank_balance")
                    local newBalance = tonumber(currentBalance) + tonumber(amount)

                    player:setData("Roleplay:bank_balance", newBalance)
                    account:setData("Roleplay:bank_balance", newBalance)

                    player:takeMoney(tonumber(amount))
                    outputDebugString("* " .. player:getName() .. " ha depositado: $" .. convertNumber(amount) .. " ", 0, 0, 150, 0)
                    player:outputChat("* Has depositado la cantidad: #004500$" .. convertNumber(amount) .. " dólares.", 0, 150, 0, true)

                    local f = getRealTime()
                    insert("insert into `BancoLog` VALUES (?,?,?,?)", convertNumber(amount), "Deposito", f.monthday .. "/" .. f.month + 1 .. "/" .. f.year - 100, AccountName(player))
                else
                    player:outputChat("* No tienes dinero para depositar", 150, 0, 0)
                end
            else
                player:outputChat("* No tienes la cantidad: " .. convertNumber(amount) .. " para depositar", 150, 0, 0)
            end
        end
    --else
        player:outputChat("No tienes una tarjeta de crédito. Ve al ayuntamiento para obtener una.", 150, 50, 50, true)
   -- end

    if not antiSpamComando[player] then
        antiSpamComando[player] = {}
    end
    antiSpamComando[player][1] = getTickCount()
end

addEvent("clientDepositMoney", true)

-- Manejar el evento y llamar a la función depositMoney
addEventHandler("clientDepositMoney", root, function(amount)
    local player = client
