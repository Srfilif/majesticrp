			...
			elseif key == "mouse1" or key == "enter" then
				if data.name[selShow] then
					...
					if items[name] then
						...
						if getPlayerMoney(localPlayer) < tonumber(cost) then
							exports.GTIhud:dm( "No tienes dinero para comprar "..seeds.." semillas.", 255, 0, 0)
							return
						end
						if tonumber(theSeeds) >= tonumber(10000) then
							exports.GTIhud:dm( "No puedes llevar mas semillas", 255, 0, 0)
							return
						end
						giveSeeds( seeds, cost)
						exports.GTIhud:dm( "Compraste "..seeds.." semillas por $"..cost, 255, 255, 0)
						uiExit( true)
					end
				end
