farmerZone1 = createColCuboid(772.83649, -453.63440, 15.42809-4, 232.98767089844, 243.98538208008, 84.31139793396)
...
local farmingRadars = {
    {772.83649, -453.63440, 232.98767089844, 243.98538208008},
}
...
function createDXViewer()
    ...
    if (not isPlayerMapVisible()) then
        if theSeeds ~= nil then
			...
			dxDrawText( theSeeds.."/10000 Seeds", 1370, 559, 1555, 584, tocolor(255, 255, 255, 255), 1.15, "default-bold", "center", "center", false, false, false, false, false)
        end
        --->> Harvest Viewer
        if not getElementData(localPlayer, "drawHarvestLeft") then
            if thePlanted ~= nil then
				...
				dxDrawText( thePlanted.."/50 Seeds Planted", 1370, 586, 1555, 611, tocolor(255, 255, 255, 255), 1.15, "default-bold", "center", "center", false, false, false, false, false)
            end
        end
    end
end
...
function fixFarmerLogin()
    if getElementData( localPlayer, "job") == "Granjero" then
        currentPlanted = 0
        thePlanted = "0"
        if not dxEnabled then
