	...
	if #scoreboardColumns ~= 0 then
				...
				if not getPlayerTeam( player ) or not (showTeams or (serverInfo.forceshowteams and not serverInfo.forcehideteams)) or serverInfo.forcehideteams then
					...
					for key, column in ipairs( scoreboardColumns ) do
						...
						if column.name == "name" then
							...
							---
								if not string.find( playerName, "#%x%x%x%x%x%x" ) then
									local colorCodes = {}
									
								--	while( string.find( playerName, "#%x%x%x%x%x%x" ) ) do
										local startPos, endPos = string.find( "#ffFFff", "#%x%x%x%x%x%x" ) 
										if startPos then
											--colorCode = string.sub( playerName, startPos, endPos )
											table.insert(colorCodes, { {231, 217, 176, 255}, startPos })
						...
						elseif column.name == "ping" then
							content = getPlayerPing( player ).." ms"
						else
		...
		if scoreboardDrawn then
			...
			if serverInfo.server and showServerInfo then
				dxDrawText( "¡Bienvenido a Majestic Roleplay!", topX+s(5), y, topX+scoreboardDimensions.width-s(10), y+dxGetFontHeight( fontscale(serverInfoFont, scoreboardScale), serverInfoFont ), cServerInfo, fontscale(serverInfoFont, s(0.75)), serverInfoFont, "left", "top", false, false, drawOverGUI )
			end
...
function scoreboardResetColumns( fromServer )
	...
	if not fromServer then
		scoreboardAddColumn( "name", 200, "Nombre" )
		scoreboardAddColumn( "ping", 40, "Latencia" )
	end
	return true
end
