...
function makeSaveGUI()
	saveStuf[1] = guiCreateWindow((resX/2) - 255, (resY/2) - 155, 509, 313, "Guardado de Eventos", false)
	...
	addEventHandler("onClientGUIClick", saveStuf[17], deleteEvent, false)
...
function warpTo()
	...
	if (name ~= "") then
		triggerServerEvent("CSTevents.Warp", root, name)
	end
end
...
addEvent("CSTevents.SendEvents", true)
addEventHandler("CSTevents.SendEvents", root, receiveEvents)

function saveEvent()
	local name = guiGetText(saveStuf[8])
	if (name ~= "") then
		triggerServerEvent("CSTevents.SaveEvent", root, name, {guiCheckBoxGetSelected(saveStuf[9]), guiCheckBoxGetSelected(saveStuf[10]), guiCheckBoxGetSelected(saveStuf[11]), guiCheckBoxGetSelected(saveStuf[12])}, guiCheckBoxGetSelected(saveStuf[20]))
	end
end
...
function loadEvent()
	local name = guiGridListGetItemText(saveStuf[18], guiGridListGetSelectedItem(saveStuf[18]), 2)
	if (name ~= "") then
		triggerServerEvent("CSTevents.LoadEvent", root, name)
		guiSetVisible(saveStuf[1], false)
...
function preDelete()
	local name = guiGridListGetItemText(saveStuf[18], guiGridListGetSelectedItem(saveStuf[18]), 2)
	if (name ~= "") then
		exports.CSTnvremisc:createYesNoWindow("CSTevents.delevcb", nil, "Delete '"..name.."' Event", "Are you sure you want to delete this event?")
	end
end
...
function deleteEvent()
	local name = guiGridListGetItemText(saveStuf[18], guiGridListGetSelectedItem(saveStuf[18]), 2)
	if (name ~= "") then
		triggerServerEvent("CSTevents.DeleteEvent", root, name)
	end	
end
addEvent("CSTevents.delevcb")
addEventHandler("CSTevents.delevcb", root, deleteEvent)
