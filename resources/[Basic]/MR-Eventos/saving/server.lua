eventsDB = dbConnect("sqlite", ":/registry.db")
local cachedEventData = {}
maxSize = 65535

dbExec(eventsDB, "CREATE TABLE IF NOT EXISTS events ('name', 'data')")
...
function getEvents()
	...
	if isObjectInACLGroup ("user."..accName, aclGetGroup("EventManager")) then
		triggerClientEvent(client, "CSTevents.SendEvents", client, getFormattedEventsTable())
	else
		outputChatBox("[ERROR] No eres Event Manager", client, 255, 0, 0)
	end
end
addEvent("CSTevents.GetEvents", true)
addEventHandler("CSTevents.GetEvents", root, getEvents)
	...
	if isObjectInACLGroup ("user."..accName, aclGetGroup("EventManager")) then
		if (not getTableIndexFromName(name)) then
			...
			if (strToSave and #strToSave <= maxSize) then
				...
				triggerClientEvent(client, "CSTevents.SendEvents", client, getFormattedEventsTable())
				outputChatBox("Added, size: "..math.floor(#strToSave).." bytes", client, 0, 255, 0)
				exports.CSTadmin:writePlayerLog("(EM) "..getPlayerName(client).." saved event '"..name.."', size: "..#strToSave.." bytes", "events", client)
			else
...
addEvent("CSTevents.SaveEvent", true)
addEventHandler("CSTevents.SaveEvent", root, saveEvent)
...
addEvent("CSTevents.LoadEvent", true)
addEventHandler("CSTevents.LoadEvent", root, preLoadEvent)
...
function deleteEvent(name)
	...
	if isObjectInACLGroup ("user."..accName, aclGetGroup("EventManager")) then
		...
		if (index) then
			...
			--exports.CSTadmin:writePlayerLog("(EM) "..getPlayerName(client).." deleted event '"..name.."'", "events", client)
		else
			outputChatBox("doesnt exist", client)
		end
	end
end
addEvent("CSTevents.DeleteEvent", true)
addEventHandler("CSTevents.DeleteEvent", root, deleteEvent)
...
function warpTo(name)
	...
	if isObjectInACLGroup ("user."..accName, aclGetGroup("EventManager")) then
		...
		if (index) then
			...
			--exports.CSTadmin:writePlayerLog("(EM) "..getPlayerName(client).." warped to "..getZoneName(pos[1], pos[2], pos[3], false)..", "..getZoneName(pos[1], pos[2], pos[3], true).." dim: "..pos[5].." int: "..pos[4], "events", client)
		end
	end
end
addEvent("CSTevents.Warp", true)
addEventHandler("CSTevents.Warp", root, warpTo)
