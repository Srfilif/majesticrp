...
addCommandHandler("cp", command_cp)
...
function onMarkerHit(hitElement, mDim)
	if (hitElement and getElementType(hitElement) == "player" and mDim and getElementInterior(hitElement) == getElementInterior(source)) then
		...
		if (pType == "hp") then
			setElementHealth(hitElement, tonumber(t[3]))
			outputChatBox("You have been given "..t[3].." health", hitElement, 0, 255, 255)
		elseif (pType == "armor") then
			setPedArmor(hitElement, t[3])
			outputChatBox("Your armor has been set to "..tonumber(t[3]), hitElement, 0, 255, 255)
		elseif (pType == "vfix") then
			if (not veh) then return end
			fixVehicle(veh)
			outputChatBox("Your vehicle has been fixed", hitElement, 0, 255, 255)
		elseif (pType == "vnitro") then
			if (not veh) then return end
			addVehicleUpgrade(veh, 1010)
			outputChatBox("Your vehicle has been given nitro", hitElement, 0, 255, 255)
		elseif (pType == "vid") then
			if (not veh) then return end
			if (getElementModel(veh) ~= t[3]) then
				setElementModel(veh, t[3])
				outputChatBox("Your vehicle has been changed to "..getVehicleNameFromModel(t[3]), hitElement, 0, 255, 255)
			end
		elseif (pType == "pct") then
			if (getPedWeapon(hitElement, 11) ~= 46) then
				giveWeapon(hitElement, 46, 1, true)
				outputChatBox("You have been given a parachute", hitElement, 0, 255, 255)
			end
		end
	end
end
...
function deletePickupViaRB(pickup)
	deletePickup(pickup)
end
addEvent("CITevents.DeletePickup", true)
addEventHandler("CITevents.DeletePickup",root, deletePickupViaRB)
