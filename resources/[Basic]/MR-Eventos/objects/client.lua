...
function createRoadblockGUI()
	roadblocksWindow = guiCreateWindow(0, (resY/2)-230, 298, 466, "Creacion de Objetos", false)
	...
	blueEdit = guiCreateEdit(192, 94, 69, 22, "B", false, vehiclesTab)
	 --exports.CSTnvremisc:setEditOnlyNumbers(redEdit, true, true, true)
	 --exports.CSTnvremisc:setEditOnlyNumbers(greenEdit, true, true, true)
	 --exports.CSTnvremisc:setEditOnlyNumbers(blueEdit, true, true, true)
	
	 addEventHandler("onClientGUIClick", destroyButton, function() exports.CSTnvremisc:createYesNoWindow("CSTevents.DestroyMyRBsCallback", nil, "Destroy My Roadblocks", "Are you sure you want to destroy your roadblocks?") end, false)
	 addEventHandler("onClientGUIClick", destroyallButton, function() exports.CSTnvremisc:createYesNoWindow("CSTevents.DestroyALLRBsCallback", nil, "Destroy All Roadblocks", "Are you sure you want to destroy ALL roadblocks?") end, false)
...
function destroyMyRBs()
	triggerServerEvent("CSTevents.RB.DestroyRoadblocks", localPlayer)
end
addEvent("CSTevents.DestroyMyRBsCallback")
addEventHandler("CSTevents.DestroyMyRBsCallback", root, destroyMyRBs)

function destroyAllRBs()
	triggerServerEvent("CSTevents.RB.DestroyALLRoadblocks", localPlayer)
end
addEvent("CSTevents.DestroyALLRBsCallback")
addEventHandler("CSTevents.DestroyALLRBsCallback", root, destroyAllRBs)
...
function roadblockGUI()
	triggerServerEvent("CSTevents.RB.CheckAdminLevel", localPlayer)
end
...
function placeRoadblock()
	...
	if (getElementType(object) == "object") then
		triggerServerEvent("CSTevents.RB.AddRoadblock", localPlayer, id, x, y, z, rx, ry, rz, dim, int)
	elseif (getElementType(object) == "vehicle") then
		local r1, g1, b1, r2, g2, b2, r3, g3, b3, r4, g4, b4 = getVehicleColor(object, true)
		triggerServerEvent("CSTevents.RB.SpawnVeh", localPlayer, id, x, y, z, rx, ry, rz, dim, int, {r1, g1, b1, r2, g2, b2, r3, g3, b3, r4, g4, b4})
	end
	destroyElement(object)	
    unbindKey("mouse2", "down", cancelRoadBlock)
    unbindKey("mouse1", "down", placeRoadblock)
				...
				if (clickedWorld and isElement(clickedWorld)) then
					if (getElementType(clickedWorld) == "object") then
						for k,v in ipairs(getElementsByType("object"), resourceRoot) do
							if (v == clickedWorld) then
								triggerServerEvent("CSTevents.RB.DestroyRoadblock", root, clickedWorld)
								return
							end
						end
					elseif (getElementType(clickedWorld) == "vehicle") then
						for k,v in ipairs(getElementsByType("vehicle"), resourceRoot) do
							if (v == clickedWorld) then
								triggerServerEvent("CSTevents.RB.DestroyVeh", root, clickedWorld)
								return
							end
						end
					end
