...
function createExplorerGUI()
	...
	expWindow = guiCreateWindow((resX/2) - 300, (resY/2) - 182, 601, 384, "Buscador de Elementos", false)
...
addEvent("CSTevents.SendElements", true)
addEventHandler("CSTevents.SendElements", root, onReceiveElements)
...
function deleteElementFunc()
	...
	if (element) then
		triggerServerEvent("CSTevents.DeleteElement", root, element)
	end
end
