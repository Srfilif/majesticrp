...
--  FILE:        CSTevents/CSTevents_warp.lua
...
config = {}
gridCategories = {
	[1] = "Control de Jugadores",
	[2] = "Control de Vehiculos",
...
EMgridControls = {
	...
	[12] = {{"   Destroy Spawner/Cars", "Destroy"}, function() triggerServerEvent("CSTevents.DelVehStuff", localPlayer) end, 4},
	[13] = {{"   Enable ramps for cars", "Enable"}, {"   Disable ramps for cars", "Disable"}, "ToggleVehicleRampSpawning", 2},
	[14] = {{"   Don't Work", "Disable"}, {"   Enable 'Quit Job'", "Enable"}, "ToggleQuitJob", 1},
	[15] = {{"   Don't Work", "Disable"}, {"   Enable 'End Shift'", "Enable"}, "ToggleEndShift", 1},
	...
	[19] = {{"   Don't Work", "Set"}, {"   Set everyone on-duty", "Set"}, "TogglePlayerWorking", 1},
	[20] = {{"   Load/Save Event", "Open Window"}, function() triggerServerEvent("CSTevents.GetEvents", root) end, 4},
	...
	[27] = {{"   Element Explorer", "Open Window"}, function() triggerServerEvent("CSTevents.GetElements", root) end, 4},
}
...
EMplayergridcontrols = {
	...
	[3] = {{"Give Cash", "Give Cash"}, nil, "GiveCash", {{0, "Amount"}, {"", "Reason"}}},
...
function(config, isEventStarted, total)
	if (not EMwindow) then
		EMwindow = guiCreateWindow((resX/2) - 370, (resY/2) - 135, 771, 271, "Sistema de Eventos", false)
		...
		EMlabels[3] = guiCreateLabel(206, 23, 71, 15, "Warps used: 0/0", false, EMwindow)
		guiLabelSetColor(EMlabels[1], 0, 255, 0)
		guiLabelSetColor(EMlabels[3], 0, 255, 0)

		EMedits[1] = guiCreateEdit(271, 44, 76, 20, "30", false, EMwindow)
		--EMedits[2] = guiCreateEdit(207, 145, 198, 18, "aa", false, EMwindow)
...
function processList(config)
	...
	for k,v in ipairs(gridCategories) do
		if (rowCount < 1) then
			guiGridListSetItemText(EMgrid, guiGridListAddRow(EMgrid), 1, v, true, false)
		end
		for key,val in ipairs(EMgridClearCategories) do
			if (k == val[3]) then
				if (rowCount < 1) then
					...
					guiGridListSetItemColor(EMgrid, row, 1, 0, 255, 0)
				else
					local text
...
addEvent("CSTevents.RefreshCData", true)
addEventHandler("CSTevents.RefreshCData", root, processList)
...
function processPlayerControls()
	for k,v in ipairs(EMplayergridcontrols) do
		...
		guiGridListSetItemColor(playerActionsGrid, row, 1, 0, 255, 0)
	end
end
...
function onActionClick()
	...
	if (index) then
		...
		if (type(func) == "string") then
			triggerServerEvent("CSTevents.Action", root, func, index, guiGetText(EMedits[6]), guiGetText(EMedits[7]))
	...
	if ((limit ~= "" )) then
		...
		if (guiCheckBoxGetSelected(EMcheckboxes[4])) then
			...
			if (count == 0) or (count == #teamJobs) then
				...
				exports.CSTtexts:output("You can forbid/only allow job/teams at the same time", 255, 0, 0)
				return
			end
		end	
		if (guiGetText(source) == "Start Event") then
			triggerServerEvent("CSTevents.PreCreateEvent", root, limit, guiCheckBoxGetSelected(EMcheckboxes[3]), guiCheckBoxGetSelected(EMcheckboxes[1]), guiCheckBoxGetSelected(EMcheckboxes[2]), tableToSend, forbid)
			hideEMwindow()
		else
			if (tonumber(guiGetText(EMedits[1]))) then
				triggerServerEvent("CSTevents.AddWarps", root, limit, guiCheckBoxGetSelected(EMcheckboxes[3]), guiCheckBoxGetSelected(EMcheckboxes[2]), tableToSend, forbid)
				hideEMwindow()
			end
		end
	end
...
function stopEvent()
	if ((limit ~= "" )) then
		triggerServerEvent("CSTevents.StopEvent", root)
		hideEMwindow()
	end
end
...
addEvent("CSTevents.ToggleClientFeature", true)
addEventHandler("CSTevents.ToggleClientFeature", root, toggleClientFeature)
...
addEvent("CSTevents.SendEventDimFeature", true)
addEventHandler("CSTevents.SendEventDimFeature", root, applyEventDimFeatures)
...
addEvent("CSTevents.RemoveEventDimFeatures", true)
addEventHandler("CSTevents.RemoveEventDimFeatures", root, removeEventDimFeatures)

function EDFCheck()
	local dim = getElementDimension(localPlayer)
	if (dim == 336) then
		if (not eventDimFeatures) then
			triggerServerEvent("CSTevents.GetEventDimFeatures", root)
		end
	else
		if (eventDimFeatures) then
			triggerServerEvent("CSTevents.GetNormalFeaturesState", root)
		end
	end
end
...
function onPlayerAction()
	...
	if (currentAction > 0 and #players > 0) then
		...
		if (type(action) == "string") then
			triggerServerEvent("CSTevents.SingleAction", root, action, tableOfPlayers, text1, text2)
...
txd = engineLoadTXD("textures/nitro.txd")
engineImportTXD(txd, 1313)
dff = engineLoadDFF("textures/nitro.dff", 1313)
engineReplaceModel(dff, 1313)

txd2 = engineLoadTXD("textures/repair.txd")
engineImportTXD(txd2, 954)
dff2 = engineLoadDFF("textures/repair.dff", 954)
engineReplaceModel(dff2, 954)

txd3 = engineLoadTXD("textures/vehiclechange.txd")
engineImportTXD(txd3, 1314)
dff2 = engineLoadDFF("textures/vehiclechange.dff", 1314)
