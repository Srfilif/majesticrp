...
function raceMarkerHit(hitElement, sameDim)
	if (hitElement == localPlayer and sameDim) then
		if (currentMarkerIndex < #positions) then
			...
			triggerServerEvent("CSTevents.SetRaceWinner", root)
		end
...
addEvent("CSTevents.CreateRaceStuff", true)
addEventHandler("CSTevents.CreateRaceStuff", root, addRaceStuff)
...
addEvent("CSTevents.DestroyRaceStuff", true)
addEventHandler("CSTevents.DestroyRaceStuff", root, destroyRaceStuff)
...
addEvent("CSTevents.UpdateMarkers", true)
addEventHandler("CSTevents.UpdateMarkers", root, updateMarkers)

function addCheck(pos, markerType, markerSize)
	mType = markerType
	mSize = markerSize
	rmarkers[pos[1]] = createMarker(pos[2], pos[3], pos[4], markerType, markerSize, 255, 255, 0, 120)
	setElementData(rmarkers[pos[1]], "textInMarker", "POPLife ~ Eventos\nPunto de Control #"..pos[1].."")
...
addEvent("CSTevents.AddCheck", true)
addEventHandler("CSTevents.AddCheck", root, addCheck)
...
addEvent("CSTevents.DelPrevCheck", true)
addEventHandler("CSTevents.DelPrevCheck", root, delCheck)