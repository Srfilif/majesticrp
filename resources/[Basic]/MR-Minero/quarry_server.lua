...
function onEnter ( thePlayer, seat, jacked )
    if isElement(source) and (seat == 0) and (exports.GTIemployment:getPlayerJob(thePlayer,true) == "Minero") and (exports.GTIrentals:getPlayerRentalVehicle(thePlayer) == source) then 
        local model = getElementModel ( source )
        if vehicles[model] then
            triggerClientEvent ( thePlayer, "GTImining.triggerMission", resourceRoot, source, model )
			exports.GTIhud:dm("Dirijete a los marcadores verdes para extraer mineral", thePlayer, 255, 255, 0)
        end
    end
end
...
addEventHandler("GTImining.getPayOffset", root, function()
    local payOff = exports.GTIemployment:getPlayerJobPayment(client, "Minero")
...
addEventHandler("GTImining.Pay", root, function(payOffset,grams)
    local pay = exports.GTIemployment:getPlayerJobPayment(client, "Minero")
    ...
    local iron = Exp / 3
    local pagaTotal = pay / 2.2

	setPlayerIron ( client, getPlayerIron ( client ) + Exp )
	exports.GTIhud:drawNote("IronAmount", "+ Iron "..math.ceil(iron), client, 99, 99, 99, 7500)
    exports.GTIemployment:modifyPlayerJobProgress(client, "Minero", grams)
    exports.GTIemployment:modifyPlayerEmploymentExp(client, Exp, "Minero")
    exports.GTIemployment:givePlayerJobMoney(client, "Minero", pagaTotal)
end)

function getPlayerIron ( player )
    local acc = getPlayerAccount ( player )
    local iron = tonumber ( getAccountData( acc, "iron" ) ) or 0
    return iron
end

function setPlayerIron ( player, amount )
    local acc = getPlayerAccount ( player )
    setAccountData( acc, "iron", tostring ( amount ) )
